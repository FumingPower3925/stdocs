...
- **Valores por defecto inteligentes** — los nombres de funciones se convierten en resúmenes, los segmentos de ruta en tags, los parámetros de ruta y un 200 se documentan solos, las rutas con seguridad documentan su 401 y el sobre de error compartido se declara una sola vez para todo el mux.
...
mux := stdocs.New(
    ...
    stdocs.WithDefaultResponse(500, APIError{}), // el sobre de error, una sola vez
)
...
| ---------------------------------- | --------------------------------------- | -------------------------------------------- |
| _(por defecto)_ (interna, ~1.6 KB) | —                                        | —                                            |
...
Las URL del CDN están fijadas a versiones exactas con hashes SRI sha384; los subpaquetes no se enlazan en tu binario salvo que los importes. Detalles del montaje incrustado: [Docs UIs](https://pkg.go.dev/github.com/FumingPower3925/stdocs#hdr-Docs_UIs).
...
`stdocs.New()` devuelve un `*stdocs.Mux` que incrusta `*http.ServeMux` y registra patrón + metadatos según registras handlers. Con la primera petición a `/docs/openapi.json`, se recorre el registro y el spec se construye y se cachea (`mux.Refresh()` lo reconstruye). Sin comentarios, sin generación de código, sin `unsafe`: el patrón es la documentación.
